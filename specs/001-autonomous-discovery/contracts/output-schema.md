# Output Contract: Phase 2 Autonomous Discovery Skill
...
Or switch to manual mode (optional fallback):
...
**Output Message**:
```
Discovery failed for topic '{topic}' (all sources unavailable).

Try again later or provide hints:
/daily-digest {topic} --hints [youtube_channels or @twitter_users]
...
**Discovery Status**: Complete (all sources)
...
- **Outcome**: Discovery time reduced from 40s to 27s; improved insight comprehensiveness
...
⚠️ **Low-signal content** — Insights below represent the best available material.
Discovery: Partial (2 of 3 sources completed)
```
